...
def test_for_statement():
    ...
    for word in numbers[:]:  # Loop over a slice copy of the entire list.
        if (word) > 1000:
    ...
    assert numbers == [5265, 100, 5265, 50]
    ...
    person = {'Fareed': 'SD', 'Hamza': 'Animator'}
    for key, value in person.items():
        names.append(key)
        properties.append(value)

    assert names == ['Fareed', 'Hamza']
    assert properties == ['SD', 'Animator']
