...
# imported module:
import factorial_module as fm

# It can also be used when utilising from with similar effects:
from factorial_module import factorial as factorial_renamed
...
def test_modules():
    ...
    assert factorial_module.factorial(7) == 7 * 6 * 5 * 4 * 3 * 2
    assert factorial(7) == 7 * 6 * 5 * 4 * 3 * 2
    assert fm.factorial(7) == 7 * 6 * 5 * 4 * 3 * 2
    assert factorial_renamed(7) == 7 * 6 * 5 * 4 * 3 * 2

    # If you intend to use a function often you can assign it to a local name.
    fac = factorial_module.factorial
    assert fac(5) == 120

    # The built-in function dir() is used to find out which names a module defines. It returns a
    # sorted list of strings.
    assert dir(factorial_module) == [
        '__builtins__',
        '__cached__',
        '__doc__',
        '__file__',
